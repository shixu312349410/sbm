...
object Axi4Ip{
  def rename(axi:Axi4): Unit ={
    val dir = if(axi.aw.valid.isOutput) "m" else "s"
    for(ax <- Seq(axi.ar, axi.aw, axi.w, axi.r, axi.b)){
      ax.flatten.foreach{bt =>
        val newName = bt.getName()
          .replace("payload_", "")
          .split("_")
          .dropWhile(_!=ax.getPartialName())
          .reduce(_++_)
        bt.setName(s"${dir}_axi_$newName")
      }
    }
  }
}
...
// https://github.com/huang-jl/LLCL-MIPS/blob/438864414a31bd92f62a9fc1d4218508bec381da/src/main/scala/ip/CrossBarIP.scala

/** m slave x n master CrossBar IP BlackBox */
case class AxiCrossbar(m: Int, n: Int) extends BlackBox {
  ...
  val io = new Bundle {
    ...
    val s_axi = new Bundle {
      ...
      val w = new Bundle {
        val data  = in Bits (64 * m bits)
        val strb  = in Bits (8 * m bits)
        val last  = in Bits (m bits)
        val valid = in Bits (m bits)
        val ready = out Bits (m bits)
      }
      val b = new Bundle {
        val id    = out Bits (3 * m bits)
    ...
    val m_axi = new Bundle {
      ...
      val w = new Bundle {
        val data  = out Bits (64 * n bits)
        val strb  = out Bits (8 * n bits)
        val last  = out Bits (n bits)
        val valid = out Bits (n bits)
        val ready = in Bits (n bits)
      }
      val b = new Bundle {
        val id    = in Bits (3 * n bits)
...
object AxiCrossbar {
  def connect(crossbar: AxiCrossbar, masters: Array[Axi4], slaves: Array[Axi4]): Unit = {
    ...
    crossbar.io.s_axi.aw.prot := masters
      .map(ele => ele.aw.prot)
      .reduceLeft((res, ele) => ele ## res)
    crossbar.io.s_axi.aw.qos := masters
      .map(ele => ele.aw.qos)
      .reduceLeft((res, ele) => ele ## res)
    ...
    crossbar.io.s_axi.ar.prot := masters
      .map(ele => ele.ar.prot)
      .reduceLeft((res, ele) => ele ## res)
    crossbar.io.s_axi.ar.qos := masters
      .map(ele => ele.ar.qos)
      .reduceLeft((res, ele) => ele ## res)
    ...
    masters.zipWithIndex.foreach(ele => {
      ...
      ele._1.b.id := crossbar.io.s_axi.b.id(ele._2 * 3, 3 bits).asUInt
      ...
      ele._1.r.id := crossbar.io.s_axi.r.id(ele._2 * 3, 3 bits).asUInt
      ele._1.r.data := crossbar.io.s_axi.r.data(ele._2 * 64, 64 bits)
    ...
    slaves.zipWithIndex.foreach(ele => {
      ele._1.aw.id := crossbar.io.m_axi.aw.id(3 * ele._2, 3 bits).asUInt
      ...
      ele._1.w.data := crossbar.io.m_axi.w.data(64 * ele._2, 64 bits)
      ele._1.w.strb := crossbar.io.m_axi.w.strb(8 * ele._2, 8 bits)
      ...
      ele._1.ar.id := crossbar.io.m_axi.ar.id(3 * ele._2, 3 bits).asUInt
...
class Axi4OnChipRam(config:Axi4Config) extends BlackBox{
  val io = new Bundle{
    val s_aclk = in Bool()
    val s_aresetn = in Bool()
    val axi = slave(Axi4(config))
  }

  noIoPrefix()
  mapClockDomain(clock = io.s_aclk, reset = io.s_aresetn, resetActiveLevel = LOW)
  Axi4Ip.rename(io.axi)
}

class Axi4DataFifo(config:Axi4Config) extends BlackBox{
  val io = new Bundle{
    val aclk = in Bool()
    val aresetn = in Bool()
    val saxi = slave(Axi4(config))
    val maxi = master(Axi4(config))
  }
  noIoPrefix()
  mapClockDomain(clock = io.aclk, reset = io.aresetn, resetActiveLevel = LOW)
  Axi4Ip.rename(io.saxi)
  Axi4Ip.rename(io.maxi)
}